...
#define EMPTY_STATE                                                            \
  {                                                                            \
    ...
    .desired_keycode = 0, .saved_mods = 0, .sequence_len = 0,                  \
    .pressed_time = 0, .released_time = 0, .timeout = INVALID_DEFERRED_TOKEN,  \
...
#define EMPTY_STATE                                                            \
  {                                                                            \
    .pressed_keyposition = MAKE_KEYPOS(0, 0), .pressed_keycode = 0,            \
    .desired_keycode = 0, .sequence_len = 0, .pressed_time = 0,                \
...
void smtd_apply_stage(smtd_state *state, smtd_stage next_stage) {
  ...
  case SMTD_STAGE_TOUCH:
    ...
    state->timeout =
        defer_exec(get_smtd_timeout_or_default(state, SMTD_TIMEOUT_TAP),
                   timeout_touch, state);
  ...
  case SMTD_STAGE_SEQUENCE:
    ...
    state->resolution = SMTD_RESOLUTION_UNCERTAIN;
    state->timeout =
        defer_exec(get_smtd_timeout_or_default(state, SMTD_TIMEOUT_SEQUENCE),
...
void smtd_handle_action(smtd_state *state, smtd_action action) {
  ...
  if (smtd_worst_resolution_before(state) < SMTD_RESOLUTION_DETERMINED) {
    SMTD_DEBUG("%s %s is deffered", smtd_state_to_str(state),
               smtd_action_to_str(action));
    return;
  }
...
void smtd_execute_action(smtd_state *state, smtd_action action) {
  ...
  */
  ...
  smtd_resolution new_resolution =
      on_smtd_action(state->desired_keycode, action, state->sequence_len);
  smtd_bypass = false;
